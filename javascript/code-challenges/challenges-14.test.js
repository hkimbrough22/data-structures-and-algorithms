...
// 'Mr. Brown', 'Dr. Blue', 'Ms. Black', 'Mrs. Orange'

const screenForNames = (arr) => {
  // let regex2 = /\bM( ?:r|?:rs?|?:s|)\b/g;
...
let biggerThanLuke = (arr) => {
  ...
  let finalArr = [];
  arr.forEach(person => {
    person.name === 'Luke Skywalker' ? massLuke = parseInt(person.mass) : null;
  });
  let heavierArr = arr.filter(person => {
    if(parseInt(person.mass) > massLuke) {
      return person;
    }});
  for(let person of heavierArr) {
    finalArr.push(person.name);
  }
  let newString = finalArr.toString().replace(/,/g, ' - ');
  return newString;
};
