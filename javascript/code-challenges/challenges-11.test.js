...
const totalSum = (input) => {
  let totalNum = 0;
  input.map( item => {
    let reducedNum = item.reduce( (acc, val) =>{
      acc += val;
      return acc;
    }, 0);
    totalNum += reducedNum;
...
// const finalArr = [];
//   input.forEach( item => {
//     let newArr = item.filter( item2 => {
//       return ((typeof(item2) === 'number') && (item2 % 5 === 0));
//     });
//     newArr.forEach( item => {
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach( person => {
    for(let property in person) {
      if(person[property] === 'male' || person[property] === 'female') {
        string += person.name + ' and ';
      }
    }
  });
  let finalString = string.slice(0, string.length - 5);
  return finalString;
};
...
// matrix.forEach(item => {
//   let reducedNum = item.reduce((acc, val) => Math.max(acc, val), 0);
//   if (reducedNum > highestNum) {
//     highestNum = reducedNum;
//   }
// });
// return highestNum;
// };
